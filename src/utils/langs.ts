export function lang(langKey: string): string {
  const langs: { [name: string]: string } = {
    ...
    games_spyfall_players_count: 'Количество игроков',
    games_spyfall_themes_header: 'Темы',
    games_spyfall_next_button: 'Я прочитал слово',
    games_spyfall_next_player_button: 'Я игрок %s',
    ...
    games_spyfall_rules: `В игре участвуют местные и шпионы. Передавайте друг другу телефон, чтобы узнать свою роль. 
                      ...
                      – Выбрали одного и того же человека? Он должен раскрыть свою роль. Если это действительно Шпион, местные победили.<br>
    ...
    games_neverihaveever_phrase_hint: 'Если делали — загибаете палец.<br>5 или 10 загнутых пальцев — выполняете наказание.',
    ...
    games_neverihaveever_rules: `Правила просты и вполне укладываются в само название игры: при нажатии кнопки "Высказывание" появляется текст, 
                                 один из игроков зачитывает его вслух. После этого все те, кто не может согласиться с произнесённым (то есть, те, 
