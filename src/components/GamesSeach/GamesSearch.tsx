...
export function GamesSearch(props: Props) {
  ...
  if (filters.gameDuration === null && filters.playersCount === null) {
    ...
    filtersLabel = lang('gamelist_search_filters_players_and_duration')
        .replace('{playersCount}', langNumeric(filters.playersCount, 'filters_players')
            .replace('%s', filters.playersCount.toString()))
        .replace('{gameDuration}', langNumeric(filters.gameDuration, 'filters_minutes')
            .replace('%s', filters.gameDuration.toString()));
  ...
  return (<div className="GamesSearch">
    <Search onChange={(e) => props.search(e.currentTarget.value.trim())}
      placeholder={lang('search_placeholder')}
      after={lang('search_cancel')}/>
