import { Button, CardGrid, CellButton, Div, Group, IOS, Panel,
  PanelHeader, PanelHeaderButton, platform, Tabs, TabsItem } from '@vkontakte/vkui';
...
export const GameSettings = (props: Props) => {
  ...
  return <Panel id={props.id} className="NeverHateIEver__panel">
    ...
    <Group>
      ...
      {activeTab === 0 &&
      <CardGrid size="m">{phrases.map((item) => <ListItemComponent
        key={'phrases' + item.id}
        item={item}
        selected={selectedPhrases}
        setSelected={setSelectedPhrases}
      />)}</CardGrid>}
      {activeTab === 1 &&
      <CardGrid size="m">{punishments.map((item) => <ListItemComponent
        key={'punishments' + item.id} item={item} selected={selectedPunishments}
        setSelected={setSelectedPunishments}/>)}</CardGrid>}

      <Div style={{ display: 'flex' }}>
        <Button
          size="l"
          disabled={selectedPhrases.length === 0 || selectedPunishments.length === 0}
          stretched
          mode="secondary"
          onClick={() => {
            doHaptic();
